...
export class ClientesComponent implements OnInit {
  ...
  salvarCliente() {
    if (this.novoCliente.id) {
      this.clientesService.atualizarCliente(this.novoCliente).subscribe(
        ...
        () => {
          this.messageService.add({ severity: 'error', summary: 'Erro', detail: 'Não foi possível atualizar o cliente.' });
        }
      );
    } else {
      this.clientesService.adicionarCliente(this.novoCliente).subscribe(
        ...
        () => {
          this.messageService.add({ severity: 'error', summary: 'Erro', detail: 'Não foi possível adicionar o cliente.' });
        }
      );
    }
  }
  ...
  excluirCliente() {
    if (this.clienteSelecionado) {
      this.clientesService.removerCliente(this.clienteSelecionado.id).subscribe(
        () => {
          this.carregarClientes();
