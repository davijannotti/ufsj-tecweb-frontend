...
export class DashboardComponent implements OnInit {
    ...
    loadClientes(): void {
        this.sistemaAgiotaService.getClientes().subscribe(
            (clientes) => {
    ...
    loadEmprestimos(): void {
        this.sistemaAgiotaService.getEmprestimos().subscribe(
            (emprestimos) => {
                // Garantir que o valor de cada empréstimo seja tratado como número
                this.emprestimos = emprestimos.map((emprestimo) => ({
                    ...emprestimo,
                    valor: Number(emprestimo.valor), // Forçar conversão para número
                }));
                this.totalRendimentos = this.emprestimos.reduce(
                    (acc, emprestimo) => acc + emprestimo.valor,
                    0,
                );

                this.checkInadimplencia(this.emprestimos);
    ...
    checkInadimplencia(emprestimos: Emprestimo[]): void {
        ...
        emprestimos.forEach((emprestimo) => {
            ...
            if (
                (emprestimo.status === 'ativo' && dataVencimento < hoje) ||
                emprestimo.status === 'atrasado'
            ) {
                // Atualiza status do empréstimo para 'atrasado'
                emprestimo.status = 'atrasado';
                this.sistemaAgiotaService
                ...
                const clienteInadimplente = this.clientes.find(
                    (cliente) => cliente.id === emprestimo.cliente.id,
                );
                if (
                    clienteInadimplente &&
                    !this.clientesInadimplentes.includes(clienteInadimplente)
                ) {
    ...
    updateGraficoInadimplentes(): void {
        ...
        this.clientesInadimplentes.forEach((cliente) => {
            ...
            const primeiroEmprestimo = this.emprestimos.find(
                (emprestimo) => emprestimo.cliente.id === cliente.id,
            );
        ...
        this.sistemaAgiotaService.deleteCliente(cliente.id).subscribe(
            () => {
                this.messageService.add({
                    severity: 'success',
                    summary: 'Sucesso',
                    detail: `Cliente ${cliente.nome} removido com sucesso!`,
                });
                this.loadClientes();
            },
            (error) => {
                this.messageService.add({
                    severity: 'error',
                    summary: 'Erro',
                    detail: 'Falha ao remover cliente',
                });
            },
        );
    }
